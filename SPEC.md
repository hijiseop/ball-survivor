...
```
shared/
  constants.js          # 서버+클라 공유 상수 (월드, 틱, 전투, 히트박스, 그리드)
...
client/
  ...
  input.js              # 마우스 입력 → 월드 좌표 변환
  renderer.js           # Canvas 렌더링 (보간 + 카메라 + HUD + 이펙트)
  hud-killfeed.js       # 킬로그 피드 (좌측 하단, fade-out)
  hud-leaderboard.js    # 실시간 리더보드 (우측 상단, 킬 수 기준)
